...
**Audience:** operators who want to participate in the OC guardian operator program *without ever touching the [`guardian.ochk.io`](https://guardian.ochk.io) portal*. This document is a first-class artifact maintained at parity with the portal — a portal feature is **not done** until its bypass is documented and verified working.
...
Every row here is a portal route the operator might land on, paired with the kit-only path that does the same thing. Both paths are supported. Both are documented at parity. The kit-only path never depends on `guardian.ochk.io` being online.

| # | Lifecycle stage | Portal path | Kit-only equivalent |
|---|---|---|---|
| 01 | Apply to the program | [guardian.ochk.io/apply](https://guardian.ochk.io/apply) | Email `apply@ochk.io` with the application questionnaire — see [§01 below](#01-apply-to-the-program) |
...
**Portal path:** fill out [guardian.ochk.io/apply](https://guardian.ochk.io/apply); WebAuthn-sign the application; submit.

**Bypass:**

1. Download the application questionnaire from [guardian.ochk.io/apply.md](https://guardian.ochk.io/apply.md) (also tracked in this repo at [`docs/application-questionnaire.md`](./docs/application-questionnaire.md)).
...
**Portal path:** [guardian.ochk.io/federations](https://guardian.ochk.io/federations) lists federations with seats open.
