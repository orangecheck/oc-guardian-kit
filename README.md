...
**The operator's actual control surface for running an OrangeCheck-affiliated Fedimint guardian.** Self-serve. Operator-controlled. Composes with `fedimintd` rather than replacing it. Optional companion to the [`guardian.ochk.io`](https://guardian.ochk.io) portal — the kit works end-to-end without ever touching the portal.
...
>
> **Property the kit guarantees:** every action runs under operator-controlled credentials on operator-controlled infrastructure. OC has no access to your hardware, your keys, your `fedimintd`, or your operator identity at any point. The portal at `guardian.ochk.io` is convenience, not control — it can request signatures from your hardware, but it cannot produce them.
...
Operators who want the portal at `guardian.ochk.io` to drive their guardian (push UI for ceremony, dashboards, signing requests) opt in via:
...
A client implies a server it talks to. The kit implies a toolbox the operator runs against their own infrastructure. There is no central server you depend on; the portal at `guardian.ochk.io` is one of many surfaces the kit can interoperate with, and operators can choose to use none of them.
